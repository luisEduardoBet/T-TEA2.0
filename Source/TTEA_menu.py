...
class MenuSettings(ctk.CTkFrame):
    def __init__(self, master):
        ...
        self.label_title = ctk.CTkLabel(
            self, text="Configurações da Interface", font=(20, 20)
        )
        self.label_title.grid(
            row=0, column=0, padx=5, columnspan=2, sticky="nsew"
        )

        self.label_scale = ctk.CTkLabel(
            self, text="Escala da Interface", font=(15, 15)
        )
        self.label_scale.grid(
            row=1, column=0, columnspan=2, padx=5, sticky="s"
        )

        self.menu_scale = ctk.CTkOptionMenu(
            self, values=["80%", "90%", "100%", "110%", "120%"], command=None
        )
        self.menu_scale.grid(row=2, column=0, columnspan=2, padx=50)

        self.label_calibration = ctk.CTkLabel(
            self, text="Calibração", font=(15, 15)
        )
        self.label_calibration.grid(
            row=3, column=0, columnspan=2, padx=5, sticky="s"
        )
...
class RegisterMenu(ctk.CTkFrame):
    def __init__(self, master):
        ...
        self.label_version = ctk.CTkLabel(
            self, text="Versão 2.0 - Release 22/03/2025", font=(10, 10)
        )
