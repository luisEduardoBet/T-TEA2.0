...
from pygame.locals import K_SPACE, KEYUP
...
class Tutorial:
    ...
    def gerenciaJogo(self):
        ...
        # senão, se passou mais do que 5 segundos e ainda não teve ajuda, dá ajuda
            ...
            pygame.draw.rect(
                self.telaTutorial.roupacerta_img,
                (0, 255, 0),
                (0, 0, 100, 100),
                10,
            )
            certo_rect = self.telaTutorial.roupacerta_img.get_rect(
                topleft=self.telaTutorial.roupacerta_pos
            )
    ...
    def verificaResultado(self):
        ...
        pygame.draw.rect(
            self.telaTutorial.roupacerta_img, (0, 255, 0), (0, 0, 100, 100), 10
        )
        certo_rect = self.telaTutorial.roupacerta_img.get_rect(
            topleft=self.telaTutorial.roupacerta_pos
        )
        ...
        if self.nivel == 3:
            pygame.draw.rect(
                self.telaTutorial.roupaerrada_img,
                (255, 0, 0),
                (0, 0, 100, 100),
                10,
            )
            erro_rect = self.telaTutorial.roupaerrada_img.get_rect(
                topleft=self.telaTutorial.roupaerrada_pos
            )
    ...
    def acoesColisao(self, x, y):
        ...
        # pintar o quadrado
        self.desafioTutorial.labirinto = (
            self.desafioTutorial.mudaParedeAtingida(
                x, y, self.desafioTutorial.labirinto
            )
        )
    ...
    def carregaTelaTutorialPausa(self):
        ...
        if self.botao_sair.criar(self.superficie):
            arq.grava_Detalhado(
                0, self.nivel, 0, "Acao profissional", "Botao Sair para Menu"
            )
