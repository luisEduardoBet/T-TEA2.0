...
class Parallelogram (Geometry): 
    ...
    def draw(self, screen):
        pygame.draw.rect(screen, self.color, ((self.x - self.side1/2, self.y - self.side2/2), (self.side1, self.side2)), self.width); 
...
class Triangle(Geometry): 
    def __init__(self, color, height, pos):
        ...
        self.v1 = (self.x + self.radius * cos(0.5 * pi), self.y - self.radius * sin(0.5 *pi)) 
        self.v2 = (self.x + self.radius * cos(210*pi/180), self.y -  self.radius * sin(210*pi/180))  
        self.v3 = (self.x + self.radius* cos(330*pi/180), self.y -  self.radius * sin(330*pi/180))

    def draw(self, screen):
        pygame.draw.polygon(screen, self.color, [self.v2,self.v3,self.v1], self.width)
