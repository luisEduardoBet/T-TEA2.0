...
class Background:
    ...
    def background_menu(self):
        self.image = image.load(
            KarteaPathConfig.kartea_image("background_menu.png"),
    ...
    def draw(self, surface):
        ...
        for n in range(startPos + 300, startPos, -1):
            ...
            if self.lines[n % self.N].target != None:
                ...
                self.lines[n % self.N].target.att_current_pos(
                    self.lines[n % self.N].target.current_pos[0],
                    self.lines[n % self.N].Y,
                )
