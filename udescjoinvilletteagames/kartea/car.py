...
class Car:
    def __init__(self):
        ...
        self.orig_image = image.load(
            KarteaPathConfig.kartea_image("carro.png"),
            size=(CAR_SIZE, CAR_SIZE),
        )
    ...
    def on_target(
        self, targets
    ):  # return a list with all targets that collide with the hand hitbox
        return [
            target for target in targets if self.rect.colliderect(target.rect)
        ]

    def kill_targets(
        self, surface, targets, score, sounds
    ):  # will kill the targets that collide with the hand when the left mouse button is pressed
